...
# To format code
Rscript -e "styler.quote::style_pkg()"
```

### Coding

* Always run `styler.quote::style_pkg()` after generating code
...
- Avoid `expect_true()` and `expect_false()` in favor of a specific expectation which will give a better failure message. A few expectations in newer releases that you might not know about are `expect_all_true()`, `expect_all_equal()`, and `expect_r6_class()`.
