...
`wf` manages the building blocks of AI coding agents: skills, agents,
rules, and hooks. Each type lives in a conventional directory
(e.g. `.claude/skills/` for Claude Code skills) and can be installed
...
Each type has its own `*_path()` function for looking up the
...
All path functions also accept `scope = 'global'` for user-wide
installation, and auto-detect the agent from `WF_AGENT` when called
without arguments.
...
For repos that bundle multiple skills under a `skills/` subdirectory,
...
#> 1 87093f6e8a2ad2aed15eda84f88ca01eedbcfd7a             TRUE
update_skills()
#> Installed skill "cran-extrachecks" to '.claude/skills/cran-extrachecks'.
```
...
#>                                          url stars
#> 1 https://github.com/christopherkenny/skills     7

tmp <- tempfile()
init_skill('my-skill', tmp)
#> Created skill "my-skill" at
#> 'C:/Users/chris/AppData/Local/Temp/RtmpwJJUid/file2c0c58d84d95/my-skill'.
```

## Agents

Custom subagents are single `.md` files with YAML frontmatter. Install
one from GitHub:
...
#> 'C:/Users/chris/AppData/Local/Temp/RtmpwJJUid/file2c0c5d4a7a/my-agent.md'.
```

## Rules

Rules are Markdown files that set coding guidelines for every session.
Install from GitHub:
...
#> 'C:/Users/chris/AppData/Local/Temp/RtmpwJJUid/file2c0c28277af9/my-rule.md'.
```

## Hooks

Hooks are executable scripts (`.sh`, `.R`, etc.) that run on agent
lifecycle events. Installing a hook copies the script to
...
#> 'C:/Users/chris/AppData/Local/Temp/RtmpwJJUid/file2c0c102173a3/my-hook.sh'.
