...
test_that('resolve_skill_path returns literal path for unknown string', {
  expect_identical(resolve_skill_path('/tmp/my/skills'), '/tmp/my/skills')
  expect_identical(resolve_skill_path('some/custom/path'), 'some/custom/path')
})
...
test_that('resolve_skill_path aborts when path is NULL and no env var set', {
  ...
  withr::with_dir(tmp, {
    withr::with_envvar(c(WF_AGENT = ''), {
      expect_snapshot(resolve_skill_path(NULL), error = TRUE)
    })
  })
})
